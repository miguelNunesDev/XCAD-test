...
const PlatformPriceCard = ({ className }: Props) => {
    ...
    if (!platformContext) return <>Error</>;
    const {
    ...
    useEffect(()=>{
        globalContext.price = Number(platformsState.price.toFixed(3));
    },[platformsState.price])
    ...
    return (
        <div className={`relative ${className}`}>
            ...
            <section
                className={`relative inline-block h-40 w-72 ${isLoading
                    ? 'transition-all blur-[2px] saturate-0 pointer-events-none'
                    : ''
                    }`}
            >
